...
from math import isfinite, isnan, nextafter
...
_ENABLE_A2_EVENT_ROW_ELIMINATION = True
_ENABLE_A3_BOUND_TIGHTENING = True


def _round_down(value: float) -> float:
    """One-ULP outward rounding toward negative infinity."""
    value = float(value)
    if value == -np.inf:
        return value
    if isnan(value):
        raise ValueError("interval arithmetic produced NaN")
    return nextafter(value, -np.inf)


def _round_up(value: float) -> float:
    """One-ULP outward rounding toward positive infinity."""
    value = float(value)
    if value == np.inf:
        return value
    if isnan(value):
...
def _interval_add(left: tuple[float, float], right: tuple[float, float]) -> tuple[float, float]:
    return _round_down(left[0] + right[0]), _round_up(left[1] + right[1])


def _interval_sub(left: tuple[float, float], right: tuple[float, float]) -> tuple[float, float]:
    return _round_down(left[0] - right[1]), _round_up(left[1] - right[0])


def _interval_scale(interval: tuple[float, float], coefficient: float) -> tuple[float, float]:
    coefficient = float(coefficient)
    if coefficient == 0.0:
        return 0.0, 0.0
    if coefficient > 0.0:
        return _round_down(interval[0] * coefficient), _round_up(interval[1] * coefficient)
    return _round_down(interval[1] * coefficient), _round_up(interval[0] * coefficient)


def _divide_down(value: float, positive_divisor: float) -> float:
    if not positive_divisor > 0.0:
        raise ValueError("interval divisor must be positive")
    return _round_down(value / positive_divisor)


def _divide_up(value: float, positive_divisor: float) -> float:
    if not positive_divisor > 0.0:
        raise ValueError("interval divisor must be positive")
    return _round_up(value / positive_divisor)
...
def _solve_joint_experimental(
    ...
    *,
    ...
    # Precompute the cumulative event structure once.  A propagation node is
    # one QH/event prefix and therefore exactly one original position row.
    event_nodes_by_qh: dict[str, list[tuple[tuple[int, float], ...]]] = {}
    event_nodes: list[tuple[str, tuple[tuple[int, float], ...]]] = []
    nodes_by_contract: list[set[int]] = [set() for _ in contracts]
    for qh in qhs:
        cumulative: list[tuple[int, float]] = []
        qh_nodes: list[tuple[tuple[int, float], ...]] = []
        for event_time in sorted(contract_events_by_qh[qh.qh_id]):
            cumulative.extend(contract_events_by_qh[qh.qh_id][event_time])
            terms = tuple(cumulative)
            qh_nodes.append(terms)
            node_id = len(event_nodes)
            event_nodes.append((qh.qh_id, terms))
            for j, _ in terms:
                if net_index[j] >= 0:
                    nodes_by_contract[j].add(node_id)
        event_nodes_by_qh[qh.qh_id] = qh_nodes

    # A3: propagate every event-node position interval into individual signed
    # contract bounds.  Prefix/suffix interval sums evaluate all "other term"
    # bounds in linear time per node.  A dependency queue revisits only nodes
    # touched by an actual bound change.  Every primitive multiplication,
    # addition/subtraction and division remains directionally rounded.
    original_net_bounds = {
        j: (lower_list[int(net_index[j])], upper_list[int(net_index[j])])
        for j in range(n_contracts) if net_index[j] >= 0
    }
    a3_contract_started = perf_counter()
    a3_node_visits = 0
    a3_nodes_initially_skipped = 0
    if _ENABLE_A3_BOUND_TIGHTENING:
        initial_nodes: list[int] = []
        for node_id, (qh_id, terms) in enumerate(event_nodes):
            has_free_quantity = any(net_index[j] >= 0 for j, _ in terms)
            needs_propagation = (
                has_free_quantity and fixed_for_qh[qh_id].baseline_mw != 0.0
            )
            if has_free_quantity and not needs_propagation:
                for j, weight in terms:
                    index = int(net_index[j])
                    if (
                        weight > 1.0
                        or (index < 0 and net_fixed[j] != 0.0)
                        or (
                            index >= 0
                            and (lower_list[index] != -100.0 or upper_list[index] != 100.0)
                        )
                    ):
            ...
            if needs_propagation:
                ...
                # With zero fixed position, weights <= 1 and every free
                # quantity in [-100,100], an individual event row cannot
                # tighten any one free quantity.  A later dependency change
                # still queues this node through nodes_by_contract.
                a3_nodes_initially_skipped += 1
        pending = deque(initial_nodes)
        queued = set(pending)
        while pending:
            node_id = pending.popleft()
            queued.remove(node_id)
            a3_node_visits += 1
            qh_id, terms = event_nodes[node_id]
            term_intervals: list[tuple[float, float]] = []
            for j, weight in terms:
                index = int(net_index[j])
                interval = (
                    (lower_list[index], upper_list[index])
                    if index >= 0 else (float(net_fixed[j]), float(net_fixed[j]))
                )
                term_intervals.append(_interval_scale(interval, weight))
            prefix = [(float(fixed_for_qh[qh_id].baseline_mw), float(fixed_for_qh[qh_id].baseline_mw))]
            for term_interval in term_intervals:
                prefix.append(_interval_add(prefix[-1], term_interval))
            suffix = [(0.0, 0.0)] * (len(terms) + 1)
            for position in range(len(terms) - 1, -1, -1):
                suffix[position] = _interval_add(term_intervals[position], suffix[position + 1])
            changed_contracts: set[int] = set()
            for position, (j, weight) in enumerate(terms):
                index = int(net_index[j])
                if index < 0 or weight <= 0.0:
                    continue
                other_interval = _interval_add(prefix[position], suffix[position + 1])
                lower_numerator = _interval_sub(
                    (-100.0, -100.0), (other_interval[1], other_interval[1])
                )[0]
                upper_numerator = _interval_sub(
                    (100.0, 100.0), (other_interval[0], other_interval[0])
                )[1]
                new_lo = max(lower_list[index], _divide_down(lower_numerator, weight))
                new_hi = min(upper_list[index], _divide_up(upper_numerator, weight))
                if new_lo > new_hi:
                    forced_infeasible = True
                    new_lo = new_hi = min(max(0.0, new_hi), new_lo)
                if new_lo > lower_list[index] or new_hi < upper_list[index]:
                    lower_list[index] = new_lo
                    upper_list[index] = new_hi
                    changed_contracts.add(j)
            for j in changed_contracts:
                for dependent_node in nodes_by_contract[j]:
                    if dependent_node not in queued:
                        pending.append(dependent_node)
                        queued.add(dependent_node)
    a3_contract_seconds = perf_counter() - a3_contract_started

    # Tighten reserve upper bounds with the reachable baseline interval after
    # the contract/event propagation above.  The bound is an existential
    # consequence of baseline+up<=export and -baseline+down<=import.
    a3_remaining_started = perf_counter()
    reserve_bounds_tightened = 0
    baseline_interval_by_qh: dict[str, tuple[float, float]] = {}
    baseline_exact_by_qh: dict[str, float] = {}
    baseline_determined_by_qh: dict[str, bool] = {}
    for i, qh in enumerate(qhs):
        fc = fixed_for_qh[qh.qh_id]
        baseline_interval = (0.0, 0.0)
        baseline_exact = 0.0
        baseline_determined = True
        for j, coefficient in qh_market_terms[qh.qh_id]:
            index = int(net_index[j])
            if index >= 0:
                term_interval = (lower_list[index], upper_list[index])
                baseline_determined = False
            else:
                value = float(net_fixed[j])
                term_interval = (value, value)
                baseline_exact += value * coefficient
            baseline_interval = _interval_add(
                baseline_interval,
                _interval_scale(term_interval, coefficient),
            )
        baseline_interval_by_qh[qh.qh_id] = baseline_interval
        baseline_exact_by_qh[qh.qh_id] = baseline_exact
        baseline_determined_by_qh[qh.qh_id] = baseline_determined
        if not _ENABLE_A3_BOUND_TIGHTENING:
            continue
        up_index = int(reserve_up_index[i])
        if up_index >= 0 and lower_list[up_index] != upper_list[up_index]:
            candidate_interval = (inp.grid_export_mw, inp.grid_export_mw)
            candidate_interval = _interval_sub(candidate_interval, (fc.baseline_mw, fc.baseline_mw))
            candidate_interval = _interval_sub(candidate_interval, (fc.reserve_up_mw, fc.reserve_up_mw))
            candidate_interval = _interval_sub(candidate_interval, (baseline_interval[0], baseline_interval[0]))
            candidate = candidate_interval[1]
            tightened = min(upper_list[up_index], float(candidate))
            if tightened < 0.0:
                forced_infeasible = True
                tightened = 0.0
            reserve_bounds_tightened += tightened < upper_list[up_index]
            upper_list[up_index] = tightened
        down_index = int(reserve_down_index[i])
        if down_index >= 0 and lower_list[down_index] != upper_list[down_index]:
            candidate_interval = (inp.grid_import_mw, inp.grid_import_mw)
            candidate_interval = _interval_add(candidate_interval, (fc.baseline_mw, fc.baseline_mw))
            candidate_interval = _interval_sub(candidate_interval, (fc.reserve_down_mw, fc.reserve_down_mw))
            candidate_interval = _interval_add(candidate_interval, (baseline_interval[1], baseline_interval[1]))
            candidate = candidate_interval[1]
            tightened = min(upper_list[down_index], float(candidate))
            if tightened < 0.0:
                forced_infeasible = True
                tightened = 0.0
            reserve_bounds_tightened += tightened < upper_list[down_index]
            upper_list[down_index] = tightened

    net_bounds_tightened = (
        sum(
            lower_list[int(net_index[j])] > bounds[0] or upper_list[int(net_index[j])] < bounds[1]
            for j, bounds in original_net_bounds.items()
        ) if _ENABLE_A3_BOUND_TIGHTENING else 0
    )

    # A1/A3: derive a reachable power interval for every physical event.  A QH
    # whose spot and reserve quantities are all constants has one exact power
    # at every phase, so charge, discharge and direction can be substituted.
    # For all other QHs, the interval supplies the smallest proven directional
    # big-M compatible with current market and reserve bounds.
    ...
    charge_fixed = np.zeros(n_steps)
    discharge_fixed = np.zeros(n_steps)
    operation_mode_fixed = np.zeros(n_steps)
    step_charge_bound = np.zeros(n_steps)
    step_discharge_bound = np.zeros(n_steps)
    fixed_physical_step = np.zeros(n_steps, dtype=bool)

    def logical_interval(index: int, fixed_value: float) -> tuple[float, float]:
        return ((lower_list[index], upper_list[index]) if index >= 0 else (fixed_value, fixed_value))

    physical_started = perf_counter()
    for s, step in enumerate(steps):
        qh = qhs[step.qh_index]
        fc = fixed_for_qh[qh.qh_id]
        power_interval = _interval_add(
            (float(fc.baseline_mw), float(fc.baseline_mw)),
            baseline_interval_by_qh[qh.qh_id],
        )
        exact_power = float(fc.baseline_mw) + baseline_exact_by_qh[qh.qh_id]
        determined = baseline_determined_by_qh[qh.qh_id]
        if step.phase == "up":
            index = int(reserve_up_index[step.qh_index])
            lo, hi = logical_interval(index, float(reserve_up_fixed[step.qh_index]))
            reserve_interval = _interval_add((float(fc.reserve_up_mw), float(fc.reserve_up_mw)), (lo, hi))
            power_interval = _interval_add(power_interval, reserve_interval)
            if index < 0:
                exact_power += float(fc.reserve_up_mw) + float(reserve_up_fixed[step.qh_index])
            determined &= index < 0
        elif step.phase == "down":
            index = int(reserve_down_index[step.qh_index])
            lo, hi = logical_interval(index, float(reserve_down_fixed[step.qh_index]))
            reserve_interval = _interval_add((float(fc.reserve_down_mw), float(fc.reserve_down_mw)), (lo, hi))
            power_interval = _interval_sub(power_interval, reserve_interval)
            if index < 0:
                exact_power -= float(fc.reserve_down_mw) + float(reserve_down_fixed[step.qh_index])
            determined &= index < 0
        if not _ENABLE_A3_BOUND_TIGHTENING and not determined:
            power_interval = (-charge_bound, discharge_bound)
        step_charge_bound[s] = min(charge_bound, max(0.0, -power_interval[0]))
        step_discharge_bound[s] = min(discharge_bound, max(0.0, power_interval[1]))
        fixed_physical_step[s] = determined
        if determined:
            power = exact_power
            charge_fixed[s] = max(-power, 0.0)
            discharge_fixed[s] = max(power, 0.0)
            if abs(charge_fixed[s] - charge_bound) <= _TOL:
                charge_fixed[s] = charge_bound
            if abs(discharge_fixed[s] - discharge_bound) <= _TOL:
                discharge_fixed[s] = discharge_bound
            if abs(charge_fixed[s]) <= _TOL:
                charge_fixed[s] = 0.0
            if abs(discharge_fixed[s]) <= _TOL:
                discharge_fixed[s] = 0.0
            operation_mode_fixed[s] = 1.0 if power > 0.0 else 0.0
            if charge_fixed[s] > charge_bound + _TOL or discharge_fixed[s] > discharge_bound + _TOL:
                forced_infeasible = True
            continue
        if step_charge_bound[s] > 0.0:
            charge_index[s] = allocate(0.0, step_charge_bound[s])
        if step_discharge_bound[s] > 0.0:
            discharge_index[s] = allocate(0.0, step_discharge_bound[s])
        if step_charge_bound[s] > 0.0 and step_discharge_bound[s] > 0.0:
            operation_mode_index[s] = allocate(0.0, 1.0, integer=True)
        elif step_discharge_bound[s] > 0.0:
            operation_mode_fixed[s] = 1.0
    physical_seconds = perf_counter() - physical_started
    a3_remaining_seconds = perf_counter() - a3_remaining_started
    ...
    # Eliminate SOC along every fixed-physics run.  Each eliminated value is
    # represented as either a constant or one retained predecessor SOC plus a
    # constant offset.  This preserves continuous fixed intervals, terminal
    # conditions and the exact efficiency-weighted recursion.
    soc_anchor_index = np.full(n_steps, -1, dtype=int)
    soc_offset = np.zeros(n_steps)
    for segment_indices in step_indices_by_segment.values():
        previous_anchor = -1
        previous_offset = float(inp.e_initial_mwh)
        for s in segment_indices:
            step = steps[s]
            if fixed_physical_step[s]:
                delta = (
                    inp.eta_charge * charge_fixed[s] * step.duration_hours
                    - discharge_fixed[s] * step.duration_hours / inp.eta_discharge
                )
                soc_anchor_index[s] = previous_anchor
                soc_offset[s] = previous_offset + delta
            else:
                soc_index[s] = allocate(inp.e_min_mwh, inp.e_max_mwh)
                soc_anchor_index[s] = soc_index[s]
                soc_offset[s] = 0.0
            previous_anchor = int(soc_anchor_index[s])
            previous_offset = float(soc_offset[s])

    if not terminal_hard and salvage_price_eur_per_mwh:
        for last_step in terminal_steps:
            anchor = int(soc_anchor_index[last_step])
            if anchor >= 0:
                objective_list[anchor] -= float(salvage_price_eur_per_mwh)
            objective_offset -= float(salvage_price_eur_per_mwh) * float(soc_offset[last_step])
    ...
    if objective_offset != 0.0 or not lower_list:
    ...
    def add(row: Mapping[int, float], lo: float, hi: float) -> None:
        ...
        # HiGHS presolve treats a nonzero empty-row equality as structurally
        # infeasible even when its residual is only roundoff from substituting
        # a previously feasible floating-point solution.  Canonicalize zero
        # only within the model's existing 1e-9 audit tolerance.  Material
        # empty-row contradictions (including forced infeasibility) remain.
        if not row and lo <= _TOL and hi >= -_TOL:
            lo = min(lo, 0.0)
            hi = max(hi, 0.0)
    ...
    event_rows_total = 0
    event_rows_final_kept = 0
    event_rows_intermediate_kept = 0
    event_rows_intermediate_interval_deleted = 0
    a2_started = perf_counter()
    for qh in qhs:
        fixed_base = fixed_for_qh[qh.qh_id].baseline_mw
        qh_nodes = event_nodes_by_qh[qh.qh_id]
        for event_number, node_terms in enumerate(qh_nodes):
            cumulative_terms = [
                (int(net_index[j]), float(net_fixed[j]), weight)
                for j, weight in node_terms
            ]
            event_rows_total += 1
            is_final = event_number == len(qh_nodes) - 1
            interval_proves_redundant = False
            if _ENABLE_A2_EVENT_ROW_ELIMINATION and not is_final:
                event_interval = (float(fixed_base), float(fixed_base))
                for index, fixed_value, coefficient in cumulative_terms:
                    lo, hi = logical_interval(index, fixed_value)
                    event_interval = _interval_add(
                        event_interval, _interval_scale((lo, hi), coefficient)
                    )
                interval_proves_redundant = (
                    event_interval[0] >= -100.0 and event_interval[1] <= 100.0
                )
            if not is_final and interval_proves_redundant:
                event_rows_intermediate_interval_deleted += 1
                continue
            add_logical(cumulative_terms, -100.0 - fixed_base, 100.0 - fixed_base)
            if is_final:
                event_rows_final_kept += 1
            else:
                event_rows_intermediate_kept += 1
    a2_seconds = perf_counter() - a2_started
    ...
    for s, step in enumerate(steps):
        if operation_mode_index[s] >= 0:
            add({int(charge_index[s]): 1.0, int(operation_mode_index[s]): step_charge_bound[s]}, -np.inf, step_charge_bound[s])
            add({int(discharge_index[s]): 1.0, int(operation_mode_index[s]): -step_discharge_bound[s]}, -np.inf, 0.0)
        qh = qhs[step.qh_index]
        fc = fixed_for_qh[qh.qh_id]
        power_terms = [
            (int(discharge_index[s]), float(discharge_fixed[s]), 1.0),
            (int(charge_index[s]), float(charge_fixed[s]), -1.0),
        ] + [
            (int(net_index[j]), float(net_fixed[j]), -coefficient)
            for j, coefficient in qh_market_terms[qh.qh_id]
        ]
    ...
    for s, step in enumerate(steps):
        ...
        if fixed_physical_step[s]:
            # The eliminated SOC expression must still obey its original
            # bounds.  Constant rows retain strict infeasibility when needed.
            add_logical(
                [
                    (int(soc_anchor_index[s]), 0.0, 1.0),
                    (-1, float(soc_offset[s]), 1.0),
                ],
                inp.e_min_mwh,
                inp.e_max_mwh,
            )
            ...
            soc_terms = [
                ...
                (int(charge_index[s]), float(charge_fixed[s]), -inp.eta_charge * step.duration_hours),
                (int(discharge_index[s]), float(discharge_fixed[s]), step.duration_hours / inp.eta_discharge),
            ]
            if previous is None:
                add_logical(soc_terms, inp.e_initial_mwh, inp.e_initial_mwh)
            else:
                soc_terms.append((
                    int(soc_anchor_index[previous]),
                    0.0,
                    -1.0,
                ))
                soc_terms.append((-1, float(soc_offset[previous]), -1.0))
    ...
    if terminal_hard:
        for last_step in terminal_steps:
            add_logical(
                [
                    (int(soc_anchor_index[last_step]), 0.0, 1.0),
                    (-1, float(soc_offset[last_step]), 1.0),
                ],
                inp.e_terminal_mwh,
                inp.e_terminal_mwh,
            )
        ...
        for s, step in enumerate(steps):
            if inp.qhs[step.qh_index].madrid_year == year:
                if charge_index[s] >= 0:
                    ...
                    fixed_throughput += inp.eta_charge * charge_fixed[s] * step.duration_hours / inp.cycle_denominator_mwh
                if discharge_index[s] >= 0:
                    ...
                    fixed_throughput += discharge_fixed[s] * step.duration_hours / inp.eta_discharge / inp.cycle_denominator_mwh
        add(row, -np.inf, float(budget) - fixed_throughput)
    ...
    matrix = coo_matrix(
        (matrix_data, (matrix_row, matrix_column)), shape=(len(rows), nvar), dtype=float
    ).tocsr()
    matrix_seconds = perf_counter() - matrix_started
    pre_solver_seconds = perf_counter() - core_started
    ...
    solver_seconds_internal = perf_counter() - solver_started
    ...
    charge_values = logical_values(charge_index, charge_fixed)
    discharge_values = logical_values(discharge_index, discharge_fixed)
    operation_mode_values = logical_values(operation_mode_index, operation_mode_fixed)
    soc_values = np.array([
        (float(x[int(soc_anchor_index[s])]) if soc_anchor_index[s] >= 0 else 0.0)
        + float(soc_offset[s])
    ...
    salvage = 0.0
    if not terminal_hard and salvage_price_eur_per_mwh:
        salvage = float(salvage_price_eur_per_mwh) * sum(float(soc_values[last]) - terminal_reference for last in terminal_steps)
    objective_value = gross + salvage
    soc_by_segment: dict[str, list[float]] = {}
    power_by_qh: dict[str, list[float]] = {}
    for s, step in enumerate(steps):
        soc_by_segment.setdefault(qhs[step.qh_index].segment_id, []).append(float(soc_values[s]))
        power_by_qh.setdefault(step.qh_id, []).append(float(discharge_values[s] - charge_values[s]))
    ...
    audit_x[charge] = charge_values
    audit_x[discharge] = discharge_values
    audit_x[operation_mode] = operation_mode_values
    audit_x[soc] = soc_values
    ...
    residuals.update({
        "experimental_a1_a3_enabled": 1,
        "optimization_fixed_physical_steps": int(np.count_nonzero(fixed_physical_step)),
        "optimization_charge_variables_eliminated": int(np.count_nonzero(charge_index < 0)),
        "optimization_discharge_variables_eliminated": int(np.count_nonzero(discharge_index < 0)),
        "optimization_direction_binaries_eliminated": int(np.count_nonzero(operation_mode_index < 0)),
        "optimization_soc_variables_eliminated": int(np.count_nonzero(soc_index < 0)),
        "optimization_event_rows_total": int(event_rows_total),
        "optimization_event_rows_final_kept": int(event_rows_final_kept),
        "optimization_event_rows_intermediate_kept": int(event_rows_intermediate_kept),
        "optimization_event_rows_intermediate_interval_deleted": int(event_rows_intermediate_interval_deleted),
        "optimization_net_contract_bounds_tightened": int(net_bounds_tightened),
        "optimization_reserve_bounds_tightened": int(reserve_bounds_tightened),
        "optimization_physical_bounds_tightened": int(sum(
            step_charge_bound[s] < charge_bound or step_discharge_bound[s] < discharge_bound
            for s in range(n_steps)
        )),
        "optimization_a3_event_node_visits": int(a3_node_visits),
        "optimization_a3_event_nodes_initially_skipped": int(a3_nodes_initially_skipped),
        "optimization_timing_a3_contract_seconds": float(a3_contract_seconds),
        "optimization_timing_a3_reserve_and_physical_seconds": float(a3_remaining_seconds),
        "optimization_timing_physical_substitution_seconds": float(physical_seconds),
        "optimization_timing_a2_event_rows_seconds": float(a2_seconds),
        "optimization_timing_matrix_seconds": float(matrix_seconds),
        "optimization_timing_pre_solver_seconds": float(pre_solver_seconds),
        "optimization_timing_solver_seconds": float(solver_seconds_internal),
        "optimization_timing_post_solver_through_audit_seconds": float(
            perf_counter() - solver_started - solver_seconds_internal
        ),
    })
...
def solve_joint(
    ...
    *,
    ...
    The default dispatches to the frozen pre-A1--A3 formulation so existing
    callers retain its exact variable/row layout and performance behavior.
    ``presolve`` is passed straight through to HiGHS.  It defaults to False
    because HiGHS presolve measurably hurts this model: it collapses the SOC
    chain and event-node rows, so the search needs more nodes than without it.
    See UPDATES_20260921.md for the interleaved A/B evidence.
    """

    if not isinstance(experimental_a1_a3, bool):
        raise ValueError("experimental_a1_a3 must be boolean")
    options = dict(
        time_limit_seconds=time_limit_seconds,
        mip_rel_gap=mip_rel_gap,
        presolve=presolve,
        terminal_hard=terminal_hard,
        salvage_price_eur_per_mwh=salvage_price_eur_per_mwh,
        terminal_reference_mwh=terminal_reference_mwh,
        fixed_spot_mw=fixed_spot_mw,
    ...
    baseline_residuals = dict(payload["residuals"])
    baseline_residuals.update({
        "experimental_a1_a3_enabled": 0,
        "optimization_fixed_physical_steps": 0,
        "optimization_charge_variables_eliminated": 0,
        "optimization_discharge_variables_eliminated": 0,
        "optimization_direction_binaries_eliminated": 0,
        "optimization_soc_variables_eliminated": 0,
        "optimization_event_rows_intermediate_interval_deleted": 0,
        "optimization_net_contract_bounds_tightened": 0,
        "optimization_reserve_bounds_tightened": 0,
        "optimization_physical_bounds_tightened": 0,
        "optimization_a3_event_node_visits": 0,
        "optimization_a3_event_nodes_initially_skipped": 0,
    })
    payload["residuals"] = baseline_residuals
    return SyntheticMarketResult(**payload)
